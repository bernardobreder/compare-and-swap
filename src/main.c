...
//    int n;
//    for (n = 0 ; n < 1024 ; n++) {
//        flqueue_push(job->queue, 0);
//    }
//    for (n = 0 ; n < 1024 ; n++) {
//        if (lstack_push(job->results, 0) != 0) abort();
//    }
    ...
    while (lstack_size(job->results) < job->min_results) {
        ...
        if (memcmp(output, EMPTY, job->nzeros) == 0) {
            ...
            if (lstack_push(job->results, copy) != 0) abort();
        }
...
int main()
{
    long nthreads = sysconf(_SC_NPROCESSORS_ONLN) * 4; // Linux
//    struct flqueue_t queue;
    lstack_t results;
    struct job job = {
        .results = &results,
